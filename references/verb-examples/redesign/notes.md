...
```
/* Reskin · macrostructure: Workbench · F2 sticky-scroll knobs: pinned=right, content=feature-card, steps=3
...
The `studied: no · context: redesign` flag tells the next Reskin run that the page's macrostructure was a deliberate redesign choice — `audit` will be lenient on "Specimen-fall-through" and strict on "did the redesign actually use the new fingerprint, or did it drift back".
