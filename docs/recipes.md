# Recipes — copy-paste prompts that ship Reskin output

Eight worked briefs you can paste into Claude Code, Cursor, or Codex with the Reskin skill installed. Each shows the prompt verbatim, the audience/use/tone the skill inferred, the macrostructure + theme + enrichment it picked, and a one-paragraph excerpt of the output.
...
Every recipe shares the same four lines so you can scan a brief and immediately see *which combination Reskin picks for what*:
...
- Not templates. Reskin's whole point is structural variety — duplicating a recipe verbatim is the **Specimen-fall-through** anti-pattern (gate 23 in [`SKILL.md`](../SKILL.md)).
- Not fixed picks. Two consecutive runs of recipe 00 (Coffeebox) on the same project should produce *different* macrostructures or themes — the [`.reskin/log.json`](../SKILL.md) project memory enforces this.
